/* =========================================================================
   ProjectOS — Owner Cockpit
   Static, client-only app (built to run on GitHub Pages ή τοπικά, no
   server, no accounts, no network tracking, no build tools/frameworks).

   ΑΠΟΘΗΚΕΥΣΗ (βλ. FILE SYSTEM ACCESS PERSISTENCE παρακάτω για λεπτομέρειες):
     - Browsers που υποστηρίζουν το File System Access API (Chrome, Edge,
       και άλλα Chromium): ο χρήστης συνδέει ΕΝΑ αρχείο .json στον δίσκο
       (`showSaveFilePicker`/`showOpenFilePicker`) και η εφαρμογή γράφει
       πάνω σε αυτό αυτόματα (debounced μετά από κάθε αλλαγή + περιοδικό
       interval), μέσω `FileSystemFileHandle.createWritable()`. Το handle
       αποθηκεύεται σε IndexedDB ώστε να μη χρειάζεται να ξαναδιαλέξει το
       αρχείο ο χρήστης σε κάθε άνοιγμα (με re-request permission όταν
       χρειάζεται — δες `tryRestoreFileHandle`/`reconnectPendingHandle`).
     - Browsers χωρίς υποστήριξη (π.χ. Safari, Firefox): fallback στην
       παλιά λογική χειροκίνητου "Εξαγωγή" (download) / "Εισαγωγή" (upload).
     - Σε όλες τις περιπτώσεις κρατάμε επιπλέον ένα ελαφρύ auto-save σε
       IndexedDB ως τοπικό safety-net cache (ΟΧΙ πηγή αλήθειας — απλώς
       ώστε να μη χαθούν δεδομένα αν κλείσει κατά λάθος το tab πριν
       προλάβει να γίνει το write στο δίσκο).
     - Το "μη αποθηκευμένες αλλαγές" indicator αντανακλά την πραγματική
       κατάσταση write-to-disk: το DIRTY flag καθαρίζει ΜΟΝΟ όταν η
       εγγραφή στο αρχείο (ή το χειροκίνητο Εξαγωγή) έχει πράγματι
       ολοκληρωθεί — όχι απλά όταν η αλλαγή έγινε in-memory.

   ΔΟΜΗ ΑΡΧΕΙΟΥ (blueprint — για τον επόμενο developer):
     1) APP STATE                 — το STATE object (μνήμη), seedData,
                                     loadState/saveState.
     2) FILE SYSTEM PERSISTENCE   — File System Access API (connect/write/
                                     read αρχείου) + IndexedDB (file handle
                                     persistence + safety-net cache) +
                                     fallback download/upload.
     3) UI LAYER                  — SCHEMAS, modal, router, όλες οι view*
                                     συναρτήσεις. Διαβάζουν/γράφουν πάνω
                                     στο STATE. Κάθε "write" (add/edit/
                                     delete) καλεί saveState(), που
                                     σημαδεύει "dirty" ΚΑΙ προγραμματίζει
                                     autosave στο αρχείο + safety-cache.
   ========================================================================= */
...
/* =========================================================================
   2) APP STATE — in-memory, με πραγματική αποθήκευση σε αρχείο (βλ. §3).
   =========================================================================
   ...
   STATE κανονικά, όπως πριν.
     ...
     - loadState(): τρέχει ΜΙΑ φορά στην εκκίνηση (boot) και γεμίζει τη
       STATE με demo δεδομένα (seedData) σαν αρχικό default. Το boot()
       συνεχίζει μετά προσπαθώντας να φορτώσει πραγματικά δεδομένα από
       συνδεδεμένο αρχείο ή από το safety-net cache (βλ. §3).
     - saveState(): καλείται μετά από κάθε αλλαγή. Σημαδεύει το STATE ως
       "dirty" (μη γραμμένο ακόμα στο αρχείο) ΚΑΙ προγραμματίζει (1) ένα
       debounced write στο συνδεδεμένο αρχείο, αν υπάρχει, και (2) ένα
       debounced αντίγραφο ασφαλείας στο IndexedDB.
   --------------------------------------------------------------------- */
let STATE = null;
let DIRTY = false; // true = υπάρχουν αλλαγές που δεν έχουν γραφτεί ακόμα στο αρχείο

/** Καλείται ΜΙΑ φορά στο boot(), πριν το πρώτο render(). Γεμίζει τη μνήμη
 *  με demo δεδομένα σαν αρχικό state — το boot() μετά προσπαθεί να τα
 *  αντικαταστήσει με πραγματικά δεδομένα (συνδεδεμένο αρχείο ή cache). */
...
/** Σημαδεύει "unsaved changes" και προγραμματίζει το πραγματικό γράψιμο
 *  (στο συνδεδεμένο αρχείο, αν υπάρχει, + safety-net cache σε IndexedDB).
 *  Η οθόνη ήδη ξαναζωγραφίζεται από τον καλούντα (render()) όπως πριν. */
function saveState(){
  DIRTY = true;
...
/** Το indicator δείχνει την ΠΡΑΓΜΑΤΙΚΗ κατάσταση write-to-disk: το DIRTY
 *  καθαρίζει μόνο όταν η εγγραφή (στο αρχείο ή μέσω Εξαγωγής) έχει όντως
 *  ολοκληρωθεί — βλ. writeStateToFile()/exportData(). */
function updateUnsavedIndicator(){
  document.querySelectorAll('[data-unsaved-indicator]').forEach(el=>{
    el.style.display = DIRTY ? '' : 'none';
    el.textContent = DIRTY ? (FS_HANDLE ? '● αποθήκευση σε εξέλιξη…' : '● μη αποθηκευμένες αλλαγές') : '';
  });
}
...
/* =========================================================================
   3) FILE SYSTEM ACCESS PERSISTENCE — write-in-place σε ΕΝΑ αρχείο δίσκου
   =========================================================================
   Browser support: το File System Access API (showSaveFilePicker/
   showOpenFilePicker/FileSystemFileHandle) υποστηρίζεται σήμερα σε Chrome,
   Edge, και λοιπά Chromium-based browsers (desktop + Android). ΔΕΝ
   υποστηρίζεται σε Safari ή Firefox (Σεπτέμβριος 2026) — για αυτούς τους
   browsers κάνουμε αυτόματα fallback στο παλιό download/upload flow.
   Γνωστός περιορισμός: η άδεια πρόσβασης (permission) σε ένα αρχείο ΔΕΝ
   επιμένει αυθαίρετα μεταξύ sessions σε όλους τους browsers/λειτουργικά —
   το handle το κρατάμε σε IndexedDB, αλλά συχνά χρειάζεται νέο
   `requestPermission()` (που ΑΠΑΙΤΕΙ user gesture, π.χ. click σε κουμπί)
   στο επόμενο άνοιγμα. Γι' αυτό δείχνουμε ένα κουμπί "Επανασύνδεση
   αρχείου" όποτε η άδεια δεν είναι πλέον "granted".
   --------------------------------------------------------------------- */
const FS_SUPPORTED = typeof window !== 'undefined' && 'showSaveFilePicker' in window;
let FS_HANDLE = null;      // FileSystemFileHandle συνδεδεμένο, ή null
let PENDING_HANDLE = null; // handle που βρέθηκε στο IndexedDB αλλά χρειάζεται re-request permission
let AUTOSAVE_TIMER = null;
let SAFETY_CACHE_TIMER = null;

/** Ελαφρύ IndexedDB key-value store (χωρίς εξαρτήσεις) για:
 *   - 'fileHandle': το FileSystemFileHandle του συνδεδεμένου αρχείου
 *   - 'stateCache': ένα JSON string safety-net αντίγραφο του STATE */
const IDB_NAME = 'projectos-db', IDB_STORE = 'kv';
function idbOpen(){
...
/** Debounced αντίγραφο ασφαλείας στο IndexedDB — ΟΧΙ πηγή αλήθειας, μόνο
 *  για να μη χαθούν δεδομένα αν το tab κλείσει απότομα πριν προλάβει το
 *  πραγματικό write στο αρχείο. */
function scheduleSafetyCache(){
  clearTimeout(SAFETY_CACHE_TIMER);
  SAFETY_CACHE_TIMER = setTimeout(()=>{
    idbSet('stateCache', JSON.stringify(STATE)).catch(err=> console.error('Σφάλμα τοπικής cache ασφαλείας:', err));
  }, 1200);
}
/** Debounced αυτόματο γράψιμο στο συνδεδεμένο αρχείο, λίγο μετά την
 *  τελευταία αλλαγή (ώστε να μη γράφουμε σε κάθε keystroke). */
function scheduleAutosaveToFile(){
  if(!FS_HANDLE) return;
  clearTimeout(AUTOSAVE_TIMER);
  AUTOSAVE_TIMER = setTimeout(()=>{ writeStateToFile(); }, 4000);
}
/** Επιπλέον περιοδικό safety-net interval (κάθε 8s) — αν για κάποιο λόγο
 *  ο debounce δεν προλάβει (π.χ. συνεχόμενες αλλαγές), αυτό εγγυάται ότι
 *  δεν περνάνε πάνω από λίγα δευτερόλεπτα με unsaved αλλαγές. */
function startAutosaveInterval(){
  setInterval(()=>{ if(DIRTY && FS_HANDLE) writeStateToFile(); }, 8000);
}

/** Γράφει ολόκληρο το STATE πάνω στο συνδεδεμένο FileSystemFileHandle. */
async function writeStateToFile(){
  if(!FS_HANDLE) return false;
  try{
    const perm = await FS_HANDLE.queryPermission({ mode:'readwrite' });
    if(perm !== 'granted'){
      // Δεν μπορούμε να ζητήσουμε νέα άδεια χωρίς user gesture εδώ.
      PENDING_HANDLE = FS_HANDLE;
      FS_HANDLE = null;
      updateUnsavedIndicator();
      render();
      return false;
    }
    const writable = await FS_HANDLE.createWritable();
    ...
    console.error('Σφάλμα εγγραφής στο αρχείο:', err);
    toast('Αποτυχία αυτόματης αποθήκευσης στο αρχείο.', true);
    return false;
  }
}

/** Άνοιγμα save-picker για ΝΕΟ αρχείο σύνδεσης (ή αντικατάσταση υπάρχοντος
 *  συνδεδεμένου αρχείου με άλλο). Γράφει αμέσως το τρέχον STATE εκεί. */
async function connectFile(){
  if(!FS_SUPPORTED){ toast('Ο browser δεν υποστηρίζει αποθήκευση σε αρχείο εδώ. Χρησιμοποίησε Εξαγωγή/Εισαγωγή.', true); return; }
  try{
    const handle = await window.showSaveFilePicker({
      suggestedName: 'projectos.json',
      types: [{ description:'ProjectOS JSON', accept:{ 'application/json':['.json'] } }]
    });
    FS_HANDLE = handle;
    PENDING_HANDLE = null;
    await idbSet('fileHandle', handle);
    await writeStateToFile();
    toast('Το αρχείο συνδέθηκε — οι αλλαγές θα αποθηκεύονται αυτόματα εκεί.');
    render();
  }catch(err){
    if(err && err.name !== 'AbortError'){ console.error('Σφάλμα σύνδεσης αρχείου:', err); toast('Αποτυχία σύνδεσης αρχείου.', true); }
  }
}
/** Άνοιγμα ΥΠΑΡΧΟΝΤΟΣ .json (π.χ. από προηγούμενο export) και σύνδεσή του
 *  σαν το write-in-place αρχείο εφεξής — φορτώνει το περιεχόμενό του. */
async function openAndConnectFile(){
  if(!FS_SUPPORTED){ toast('Ο browser δεν υποστηρίζει αυτή τη λειτουργία εδώ. Χρησιμοποίησε Εισαγωγή.', true); return; }
  try{
    ...
    ensureShape(imported);
    STATE = imported;
    FS_HANDLE = handle;
    PENDING_HANDLE = null;
    await idbSet('fileHandle', handle);
    DIRTY = false;
    toast('Το αρχείο συνδέθηκε και φορτώθηκε.');
    render();
  }catch(err){
    if(err && err.name !== 'AbortError'){ console.error('Σφάλμα ανοίγματος αρχείου:', err); toast('Αποτυχία ανοίγματος αρχείου (μη έγκυρο JSON;).', true); }
  }
}
/** Αποσύνδεση του συνδεδεμένου αρχείου — επιστροφή σε καθαρό download/
 *  upload mode. Δεν διαγράφει τα δεδομένα από τη μνήμη. */
async function disconnectFile(){
  FS_HANDLE = null;
  PENDING_HANDLE = null;
  await idbDelete('fileHandle');
  toast('Το αρχείο αποσυνδέθηκε. Χρησιμοποίησε Εξαγωγή/Εισαγωγή από εδώ και πέρα.');
  render();
}
/** Καλείται στο boot(): προσπαθεί να ξαναβρεί το συνδεδεμένο αρχείο από
 *  προηγούμενο session μέσω IndexedDB. Αν η άδεια είναι ήδη "granted"
 *  (χωρίς να χρειάζεται user gesture), φορτώνει απευθείας το περιεχόμενό
 *  του σαν STATE. Αλλιώς κρατάει το handle σε αναμονή (PENDING_HANDLE)
 *  ώστε το UI να δείξει κουμπί "Επανασύνδεση". */
async function tryRestoreFileHandle(){
  ...
  try{
    ...
    console.error('Αποτυχία επαναφοράς συνδεδεμένου αρχείου:', err);
  }
}
/** Κουμπί "Επανασύνδεση αρχείου" — ΑΠΑΙΤΕΙ user gesture (click), γι' αυτό
 *  δεν μπορεί να τρέξει αυτόματα στο boot(). */
async function reconnectPendingHandle(){
  if(!PENDING_HANDLE) return;
  try{
    const perm = await PENDING_HANDLE.requestPermission({ mode:'readwrite' });
    if(perm !== 'granted'){ toast('Δεν δόθηκε άδεια πρόσβασης στο αρχείο.', true); return; }
    const file = await PENDING_HANDLE.getFile();
    const imported = JSON.parse(await file.text());
    ensureShape(imported);
    STATE = imported;
    FS_HANDLE = PENDING_HANDLE;
    PENDING_HANDLE = null;
    DIRTY = false;
    toast('Το αρχείο επανασυνδέθηκε.');
    render();
  }catch(err){
    console.error('Σφάλμα επανασύνδεσης αρχείου:', err);
    toast('Αποτυχία επανασύνδεσης αρχείου.', true);
  }
}
/** Καλείται στο boot() ΜΟΝΟ αν δεν βρέθηκε συνδεδεμένο αρχείο — φορτώνει
 *  σιωπηλά το τελευταίο safety-net αντίγραφο από IndexedDB, αν υπάρχει,
 *  ώστε να μη χαθεί δουλειά από απότομο κλείσιμο tab σε προηγούμενη
 *  συνεδρία. */
async function tryRestoreSafetyCache(){
  try{
    const cached = await idbGet('stateCache');
    if(!cached) return;
    const parsed = JSON.parse(cached);
    ensureShape(parsed);
    STATE = parsed;
    DIRTY = true; // δεν έχει επιβεβαιωθεί ότι είναι γραμμένο σε αρχείο ακόμα
    toast('Ανακτήθηκαν δεδομένα από προηγούμενη μη αποθηκευμένη συνεδρία (τοπική cache).');
  }catch(err){
    // καμία cache ακόμα — φυσιολογικό στην πρώτη χρήση
  }
}

/* ---------------------------------------------------------------------
   FALLBACK — χειροκίνητο export (download) / import (upload) .json.
   Παραμένει διαθέσιμο ΠΑΝΤΑ (ακόμα και με συνδεδεμένο αρχείο) σαν επιπλέον
   αντίγραφο ασφαλείας, και είναι η ΜΟΝΗ επιλογή σε browsers χωρίς File
   System Access API υποστήριξη (π.χ. Safari, Firefox).
   --------------------------------------------------------------------- */
function exportData(){
  ...
  a.href = url;
  a.download = 'projectos-backup-' + todayISO() + '.json';
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);

  if(!FS_HANDLE){ DIRTY = false; updateUnsavedIndicator(); }
 ...
 *  ΔΕΝ επηρεάζει τυχόν συνδεδεμένο FS_HANDLE — απλή αντικατάσταση STATE. */
function importData(event){
  ...
  reader.onload = (e)=>{
    try{
      ...
      STATE = imported;
      saveState(); // νέο περιεχόμενο -> θέλει γράψιμο στο συνδεδεμένο αρχείο, αν υπάρχει
...
/** Reusable widget αποθήκευσης (buttons + "μη αποθηκευμένες αλλαγές"
 *  ένδειξη). Χρησιμοποιείται σε >1 σημεία του UI (sidebar, mobile
 *  "Περισσότερα" sheet, Settings) — το `idPrefix` κρατάει τα ids μοναδικά.
 *  Το περιεχόμενο αλλάζει ανάλογα με την κατάσταση σύνδεσης αρχείου. */
function backupControlsHtml(idPrefix){
  const fileInputHtml = `<input type="file" id="${idPrefix}-file-import" accept=".json,application/json" style="display:none">`;
  const manualBtnsHtml = `
    <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:6px;">
      <button id="${idPrefix}-btn-export">${FS_HANDLE ? 'Λήψη αντιγράφου (.json)' : 'Εξαγωγή (.json)'}</button>
  ...
  if(!FS_SUPPORTED){
    return `
      <div class="fs-note">⚠ Αυτός ο browser δεν υποστηρίζει αποθήκευση απευθείας σε αρχείο (File System Access API — π.χ. Safari/Firefox). Χρησιμοποίησε χειροκίνητη Εξαγωγή/Εισαγωγή.</div>
      <span data-unsaved-indicator style="display:${DIRTY?'':'none'};color:var(--danger,#e2564b);font-weight:600;"></span>
      ${manualBtnsHtml}
    `;
  }
  if(PENDING_HANDLE && !FS_HANDLE){
    return `
      <div class="fs-note">🔒 Το προηγούμενα συνδεδεμένο αρχείο χρειάζεται εκ νέου άδεια πρόσβασης.</div>
      <div style="margin-top:4px;"><button id="${idPrefix}-btn-reconnect">Επανασύνδεση αρχείου</button></div>
      ${manualBtnsHtml}
    `;
  }
  if(!FS_HANDLE){
    return `
      <div class="fs-note">Δεν υπάρχει συνδεδεμένο αρχείο — τα δεδομένα ζουν στη μνήμη (+ τοπική cache ασφαλείας).</div>
      <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:6px;">
        <button id="${idPrefix}-btn-connect-new">Νέο αρχείο…</button>
        <button id="${idPrefix}-btn-connect-open">Άνοιγμα αρχείου…</button>
      </div>
      ${manualBtnsHtml}
    `;
  }
  return `
    <div class="fs-note ok">💾 Αποθηκεύεται αυτόματα στο αρχείο «${esc(FS_HANDLE.name || 'projectos.json')}».</div>
    <span data-unsaved-indicator style="display:${DIRTY?'':'none'};color:var(--danger,#e2564b);font-weight:600;"></span>
    <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:6px;">
      <button id="${idPrefix}-btn-save-now">Αποθήκευση τώρα</button>
      <button id="${idPrefix}-btn-connect-new">Αλλαγή αρχείου…</button>
      <button id="${idPrefix}-btn-disconnect">Αποσύνδεση</button>
    </div>
    <div style="margin-top:6px;"><button id="${idPrefix}-btn-export">Λήψη αντιγράφου (.json)</button></div>
  `;
}
function bindBackupControls(idPrefix){
  ...
  if(trig && file) trig.addEventListener('click', ()=>{
    if(DIRTY && !confirm('Η εισαγωγή θα αντικαταστήσει τα τρέχοντα δεδομένα στη μνήμη. Έχεις μη-αποθηκευμένες αλλαγές — συνέχεια χωρίς αποθήκευση;')) return;
    file.click();
  });
  if(file) file.addEventListener('change', importData);
  const connNew = document.getElementById(idPrefix+'-btn-connect-new');
  if(connNew) connNew.addEventListener('click', connectFile);
  const connOpen = document.getElementById(idPrefix+'-btn-connect-open');
  if(connOpen) connOpen.addEventListener('click', openAndConnectFile);
  const disc = document.getElementById(idPrefix+'-btn-disconnect');
  if(disc) disc.addEventListener('click', ()=>{ if(confirm('Αποσύνδεση αρχείου; Οι μελλοντικές αλλαγές δεν θα γράφονται πια αυτόματα εκεί.')) disconnectFile(); });
  const saveNow = document.getElementById(idPrefix+'-btn-save-now');
  if(saveNow) saveNow.addEventListener('click', async ()=>{ const ok = await writeStateToFile(); if(ok) toast('Αποθηκεύτηκε στο αρχείο.'); });
  const reconn = document.getElementById(idPrefix+'-btn-reconnect');
  if(reconn) reconn.addEventListener('click', reconnectPendingHandle);
}
...
const SCHEMAS = {
  task: {
    ...
    fields: [
      ...
      {key:'depends_on', label:'Εξαρτάται από (εργασία)', type:'ref', ref:'tasks'},
...
function openModal(schemaKey, id, presets){
  ...
  if(id){
    ...
    if(presets) Object.assign(data, presets); // π.χ. project_id προ-συμπληρωμένο από το Kanban
  }
...
function fieldInput(f, value){
  ...
  if(f.type === 'ref'){
    let refList = refOptions(f.ref);
    if(f.ref === 'tasks' && MODAL_STATE && MODAL_STATE.id){
      refList = refList.filter(o => o.value !== MODAL_STATE.id); // μια εργασία δεν εξαρτάται από τον εαυτό της
    }
    const opts = refList.map(o=>`<option value="${esc(o.value)}" ${o.value===value?'selected':''}>${esc(o.label)}</option>`).join('');
...
function renderSidebar(){
  ...
  return `
    <div class="sidebar">
      <div class="brand">
        <div class="mark"><div class="dot"></div><div class="name">ProjectOS</div></div>
        <div class="sub">Owner Cockpit</div>
      </div>
      <div class="nav">${groups}</div>
      <div class="sidebar-foot">
        ${esc(STATE.business.name || 'Χωρίς όνομα επιχείρησης')} ${FS_HANDLE ? ' · αποθηκεύεται σε αρχείο' : ' · μόνο στη μνήμη (session)'}
...
function renderTopbar(){
  const t = VIEW_TITLES[ROUTE.view] || ['ProjectOS',''];
  const now = new Date();
  return `
    <div class="topbar">
      <div>
...
function render(){
  ...
  switch(ROUTE.view){
    ...
    case 'projects': contentHtml = ROUTE.tab ? viewProjectDetail(ROUTE.tab) : viewProjects(); break;
...
async function boot(){
  await loadState(); // αρχικά demo δεδομένα σαν default
  await tryRestoreFileHandle(); // αν υπάρχει συνδεδεμένο αρχείο με ήδη-δοσμένη άδεια, φορτώνει από εκεί
  if(!FS_HANDLE){
    await tryRestoreSafetyCache(); // αλλιώς, safety-net από IndexedDB (π.χ. tab έκλεισε απότομα)
  }
  parseHash();
  render();
...
/* Register service worker — ενεργοποιεί πλήρη offline λειτουργία (PWA).
   Βλ. service-worker.js: cache-first στρατηγική για index.html/app.js/
   styles.css/manifest/icons. */
if('serviceWorker' in navigator){
  window.addEventListener('load', ()=>{
    navigator.serviceWorker.register('service-worker.js').catch(err=>{
      console.error('Αποτυχία εγγραφής service worker:', err);
    });
  });
}

// Αν υπάρχουν αλλαγές που δεν έχουν ακόμα γραφτεί στο δίσκο (DIRTY),
// προειδοποιούμε πριν το tab κλείσει/ανανεωθεί. Με συνδεδεμένο αρχείο
// αυτό σπάνια χτυπάει (αυτόματο write λίγα δευτερόλεπτα μετά από κάθε
// αλλαγή) — παραμένει όμως σαν τελευταία γραμμή άμυνας.
...
/* =========================================================================
   PROJECTS — λίστα (με progress bar) + Kanban ανά έργο για τα tasks του
   ========================================================================= */
/** % ολοκληρωμένων tasks ενός έργου (βάσει task.project_id). */
...
function viewProjects(){
  ...
  return `
    ...
    <div class="panel">
      ${list.length ? `<table>
        <thead><tr><th>Τίτλος</th><th>Πελάτης</th><th>Κατάσταση</th><th>Πρόοδος</th><th>Προθεσμία</th><th class="num">Έσοδα</th><th class="num">Κόστη</th><th class="num">Περιθώριο</th></tr></thead>
        <tbody>
          ${list.map(p=>{
            const margin = (Number(p.revenue)||0) - (Number(p.costs)||0);
            const prog = projectProgress(p.id);
            return `<tr data-project-detail="${p.id}">
              ...
              <td style="min-width:120px;">
                <div class="progress-track"><div class="progress-fill" style="width:${prog.pct}%; background:${prog.pct>=100?'var(--teal)':'var(--amber)'}"></div></div>
                <div class="row-sub" style="margin-top:4px;">${prog.done}/${prog.total} tasks</div>
              </td>
...
/* ---- Kanban στήλες, μία ανά task.status ---- */
...
/** Σελίδα λεπτομερειών ενός έργου: header + progress + Kanban board με
 *  τα tasks του (project_id === projectId). Η αλλαγή κατάστασης γίνεται
 *  μέσω dropdown σε κάθε κάρτα (βλ. σημείωση στο README/summary για το
 *  γιατί προτιμήθηκε dropdown αντί για drag-and-drop σε αυτή την,
 *  mobile-first, εφαρμογή). */
function viewProjectDetail(projectId){
  const p = findById(STATE.projects, projectId);
  if(!p){
    return `
      <div class="panel"><div class="panel-body pad">
        <p class="row-sub" style="margin-bottom:12px;">Το έργο δεν βρέθηκε — ίσως διαγράφηκε.</p>
        <button class="btn btn-sm" data-href="#/projects">← Πίσω στα έργα</button>
      </div></div>
    `;
  }
  const tasks = STATE.tasks.filter(t=>t.project_id === projectId);
  ...
  const kanbanCols = KANBAN_COLUMNS.map(([status,label])=>{
    const colTasks = tasks.filter(t=>t.status===status).sort((a,b)=>(a.due_date||'9999').localeCompare(b.due_date||'9999'));
    const cards = colTasks.map(t=>{
      const overdue = t.status!=='done' && t.due_date && daysDiff(t.due_date) < 0;
      const depLabel = t.depends_on ? refLabel('tasks', t.depends_on) : null;
      return `
        <div class="kanban-card" data-open="task:${t.id}">
          <div class="kc-title">${esc(t.title)}</div>
          <div class="kc-meta">
            ${priorityBadge(t.priority)}
            ${t.due_date ? `<span class="badge ${overdue?'coral':'grey'}"><span class="bd"></span>${fmtDateShort(t.due_date)}</span>` : ''}
          </div>
          ${depLabel ? `<div class="kc-dep">⛓ μετά: ${esc(depLabel)}</div>` : ''}
          <select class="kc-status-select" data-kanban-status="${t.id}" onclick="event.stopPropagation()">
            ${KANBAN_COLUMNS.map(([v,l])=>`<option value="${v}" ${v===t.status?'selected':''}>${esc(l)}</option>`).join('')}
          </select>
        </div>
      `;
    }).join('');
    return `
      <div class="kanban-col">
        <div class="kanban-col-head"><span>${esc(label)}</span><span class="kanban-count">${colTasks.length}</span></div>
        <div class="kanban-col-body">${cards || '<div class="kanban-empty">Καμία εργασία εδώ.</div>'}</div>
      </div>
    `;
  }).join('');
  ...
  return `
    <div class="project-detail-head">
      <button class="btn btn-sm btn-ghost" data-href="#/projects">← Έργα</button>
      <div style="flex:1; min-width:180px;">
        <h2 style="font-family:var(--font-display); font-size:19px;">${esc(p.title)}</h2>
        <div class="row-sub">${esc(refLabel('customers', p.customer_id))}${p.deadline ? ' · έως '+fmtDateShort(p.deadline) : ''} · ${statusBadge(p.status, {active:['blue','Ενεργό'], done:['teal','Ολοκληρώθηκε'], paused:['grey','Σε παύση']})}</div>
      </div>
      <button class="btn btn-sm" data-open="project:${p.id}">Επεξεργασία</button>
    </div>

    <div class="panel">
      <div class="panel-body pad">
        <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
          <span style="font-weight:500; font-size:13px;">Πρόοδος έργου</span>
          <span class="mono" style="font-size:12px; color:var(--text-dim);">${prog.pct}% (${prog.done}/${prog.total} tasks)</span>
        </div>
        <div class="progress-track"><div class="progress-fill" style="width:${prog.pct}%; background:${prog.pct>=100?'var(--teal)':'var(--amber)'}"></div></div>
        <div class="kv" style="margin-top:16px;">
          <dt>Έσοδα</dt><dd>${fmtMoney(p.revenue||0)}</dd>
          <dt>Κόστη</dt><dd>${fmtMoney(p.costs||0)}</dd>
          <dt>Περιθώριο</dt><dd style="color:${margin>=0?'var(--teal)':'var(--coral)'}">${fmtMoney(margin)}</dd>
        </div>
      </div>
    </div>

    <div class="toolbar">
      <div class="stats">${tasks.length} εργασίες σε αυτό το έργο</div>
      <button class="btn btn-primary" data-add-project-task="${p.id}">+ Νέα εργασία</button>
    </div>
      ...
      <div class="panel">
        ...
        <div class="panel-body pad">
          <p class="row-sub" style="margin-bottom:12px;">
            ${FS_SUPPORTED
              ? 'Σύνδεσε ένα αρχείο .json στον δίσκο σου — η εφαρμογή θα γράφει αυτόματα εκεί σε κάθε αλλαγή, χωρίς να χρειάζεται χειροκίνητο export. Αν ο browser δεν το υποστηρίζει, χρησιμοποίησε Εξαγωγή/Εισαγωγή.'
              : 'Αυτός ο browser δεν υποστηρίζει write-in-place αποθήκευση σε αρχείο — τα δεδομένα ζουν στη μνήμη (+ τοπική cache ασφαλείας). Πάτησε <strong>Εξαγωγή</strong> για να κατεβάσεις ένα .json αρχείο με όλα τα δεδομένα, και <strong>Εισαγωγή</strong> για να συνεχίσεις από ένα παλιότερο αρχείο.'}
          </p>
...
function bindViewEvents(){
  ...
  // project row -> Kanban detail page (#/projects/<id>)
  document.querySelectorAll('[data-project-detail]').forEach(el=>{
    el.addEventListener('click', ()=>{ location.hash = '#/projects/' + el.getAttribute('data-project-detail'); });
  });
  // Kanban κάρτα: αλλαγή status μέσω dropdown (χωρίς άνοιγμα του task)
  document.querySelectorAll('[data-kanban-status]').forEach(sel=>{
    sel.addEventListener('change', ()=>{
      const id = sel.getAttribute('data-kanban-status');
      const t = findById(STATE.tasks, id);
      if(t){ t.status = sel.value; saveState(); render(); }
    });
  });
  // "+ Νέα εργασία" μέσα από τη σελίδα έργου -> προ-συμπληρωμένο project_id
  document.querySelectorAll('[data-add-project-task]').forEach(btn=>{
    btn.addEventListener('click', ()=>{ openModal('task', null, { project_id: btn.getAttribute('data-add-project-task') }); });
  });
  // search inputs
